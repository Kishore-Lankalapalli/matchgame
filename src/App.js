...
const ran = Math.ceil(Math.random() * initialImagesList.length - 1)
...
class App extends Component {
  ...
  onChooseImage = imageId => {
    ...
    if (imageId === initialImagesList[random].id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        random: Math.ceil(Math.random() * initialImagesList.length - 1),
      }))
    } else {
  ...
  resultView = () => {
    ...
    return (
      <div className="result-container">
        ...
        <p className="your-score-text score">{score}</p>
        <button
        ...
        >
          ...
          />
          <p className="reset-button-text">PlayAgain</p>
        </button>
      </div>
    )
  }
  ...
  render() {
    const {tabId, timesUp, score, timer, random} = this.state

    const randomImage = initialImagesList[random].imageUrl

    const filteredImagesList = initialImagesList.filter(
          ...
          <div className="scorecard-container">
            ...
            <div className="timer-container">
              <img
                ...
                alt="timer"
              />
              <p className="span-score">{timer} sec</p>
            </div>
          </div>
        </nav>
